...
public class NotifyCenter {
    ...
    private static boolean publishEvent(final Class<? extends Event> eventType, final Event event) {
        ...
        //根据InstancesChangeEvent事件类型，获取相应的 CanonicalName，就是topic
        final String topic = ClassUtils.getCanonicalName(eventType);
        //将CanonicalName作为Key，从NotifyCenter#publisherMap中获取对应的事件发布者（EventPublisher）
        //在NotifyCenter中它维护了事件名称和事件发布者的关系，而默认的事件发布者为DefaultPublisher
        EventPublisher publisher = INSTANCE.publisherMap.get(topic);
        if (publisher != null) {
            // EventPublisher发布InstancesChangeEvent事件
